	...
	char*				trace)			
{
	...
	root->ann_trace(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim), 0, trace);
}
...
void ANNkd_split::ann_trace(ANNdist box_dist, int level, char* trace)
{
										// check dist calc term condition
	if (ANNmaxPtsVisited != 0 && ANNptsVisited > ANNmaxPtsVisited) return;
	...
	if (cut_diff < 0) {					// left of cutting plane
		trace[level] = 1;
		child[ANN_LO]->ann_trace(box_dist, level+1, trace);// visit closer child first
	}
	else {								// right of cutting plane
		trace[level] = 2;
		child[ANN_HI]->ann_trace(box_dist, level+1, trace);// visit closer child first
	}
...
void ANNkd_leaf::ann_trace(ANNdist box_dist, int level, char* trace)
{
	trace[level] = '\0';
	return;
}
