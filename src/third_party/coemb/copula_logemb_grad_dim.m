function [phi,psi,lik] = copula_logemb_grad_dim(pxy0,dim_list,max_repeats)
%
%
%
...
for curr_dim = dim_list
  for(i_repeat=1:max_repeats)
    ...
    oprod = cg_params.px0*cg_params.py0;
    cg_params.lik0 = sum(pxy0(:).*log(oprod(:)));
    opts = -1*zeros(1,9);
    x = conj_grad('copula_logemb_grad_fast',cg_params,x0,opts);
