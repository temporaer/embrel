function [x,histout,costdata] = bfgswopt(x0,f,par,tol,maxit,hess0)
%
...
itc=1; xc=x0;
%Orig maxarm=10; nsmax=50; debug=1;
maxarm=5; nsmax=5; debug=1;
%
n=length(x0);
userhess=0; 
if nargin == 6
...
while(norm(gc) > tol & itc <= maxit)
    ...
    %
    % Amir next two lines
    %lambda=min(1000,100/(1 + norm(gc)));
    %xt=xc+lambda*dsd; ft=feval(f,xt,par); 
    numf=numf+1;
    itc=itc+1; 
    old=0;
    if old==0 
        ...
        if 1 %ft > goal
%            [xt,iarm,lambda]=polyline(xc,fc,gc,dsd,ft,f,maxarm,par);
            lpar = [32.0000    2.0000    0.1000    0.0100    5.0000];
...
%        iarm=0; goalval=.0001*(dsd'*gc);
        ...
        while(ft > fc + lambda*goalval )
            ...
            xt=xc+lambda*dsd;
            ft=feval(f,xt,par); qc=ft; numf=numf+1;
            if(iarm > maxarm) 
    ...
    xc=xt; [fc,gc]=feval(f,xc,par); y = gc-y; numf=numf+1; numg=numg+1;
    %
