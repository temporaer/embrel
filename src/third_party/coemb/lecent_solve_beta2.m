...
%        centering_grad(diag_indic) = -1./x(diag_indic);
...
%        hessphi = hessphi + rand(size(hessphi))*1e-3;
        ...
        phi = t*f - log(det(xmat)); %-sum(log(x_diag_indic)); 
        s = 1;
        for lsiters = 1:MAXLSITERS
            newf = log(sum(exp(y+s*dy)))-dot(c,(x+s*dx));
            newmat = vec_to_smat(x+s*dx,F);
            
            if (min(eig(newmat))> 0) ; %& x(diag_indic)>0
                newphi = t*newf - log(det(newmat)); %-sum(log(x_diag_indic)); 
                if (newphi < phi )
                    break;
                end
            end
