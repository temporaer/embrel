...
%    x = conj_grad('condcode_grad',cg_params,x0,opts);
    x0 = conj_grad('copula_logemb_grad',cg_params,x0,opts);
    
    fmin_options = optimset('GradObj','on','Display','iter','MaxIter',1000,'MaxFunEvals',10000,'TolFun',1e-8,'MaxFunEvals',1e6,'GradConstr','on','LargeScale','on');

    x=fmincon('copula_logemb_grad',x0,[],[],[],[],[],[],'marg_model_constr',fmin_options,cg_params)    
