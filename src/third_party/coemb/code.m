function [phi,psis,as,bs,liks] = code(pxy0s,dim_list,options)
...
if b_reverse & ~isempty(phi_init) & ~isempty(psi_init)
   tmppsi = psi_init;
   tmpphi = phi_init;
   phi_init = tmppsi{1};
   psi_init{1} = tmpphi;
...
if b_reverse 
  ...
  if ~isempty(options.pxx0)
       pyy0 = options.pxx0; pyy0 = pyy0/sum(pyy0(:));
       w_pyy0 = options.w_pxx0;  
  else 
       pyy0='';
  end 
  if ~isempty(options.pyy0)
     pxx0 = options.pyy0; pxx0 = pxx0/sum(pxx0(:));
     w_pxx0 = options.w_pyy0;  
  else
     pxx0 = '';
  end
end
...
for curr_dim = dim_list
    for(i_repeat=1:n_restarts)
        ...
        cg_params.NYs = NY;
	if isempty(phi_init)
	  phi0 = 2*(0.5-rand(NX,curr_dim))*noise;
	else
	  phi0 = phi_init;
	end
	...
	cg_params.w_pyy0 = w_pyy0;
	    ...
	    if isempty(psi_init)
	      psi0 = 2*(0.5-rand(cg_params.NYs(i),curr_dim))*noise; 
	    else
	      psi0 = psi_init{i};
	    end
