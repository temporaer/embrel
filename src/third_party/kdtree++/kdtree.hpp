...
namespace KDTree
{
  ...
  template <size_t const __K, typename _Val,
    ...
    protected:
      ...
      typedef _Node_compare<_Val, _Acc, _Cmp> _Node_compareT;

      typedef _Region<__K, _Val, typename _Acc::result_type, _Acc, _Cmp>
        _RegionT;

    public:
      typedef _RegionT Region;
...
*/
      ...
      size_t
        count_within_range(const_reference __V, subvalue_type const __R) const throw ()
        {
          ...
          _RegionT __region(__V, __R, _M_acc, _M_cmp);
          return this->count_within_range(__region);
        }

      size_t
        count_within_range(_RegionT const& __REGION) const throw ()
        {
          if (!_M_get_root()) return 0;

          _RegionT __bounds(__REGION);
      ...
      template <typename SearchVal, class Visitor>
        ...
        visit_within_range(SearchVal V, subvalue_type const R, Visitor visitor) const throw ()
        {
          ...
          _RegionT region(V, R, _M_acc, _M_cmp);
          return this->visit_within_range(region, visitor);
        }

      template <class Visitor>
        Visitor
        visit_within_range(_RegionT const& REGION, Visitor visitor) const throw ()
        {
          if (_M_get_root())
            {
              _RegionT bounds(REGION);
      ...
      find_within_range_iterative(const_reference __a, const_reference __b)
	{
      ...
      template <typename _OutputIterator>
        _OutputIterator
        find_within_range(_RegionT const& __REGION,
                          _OutputIterator __out) const throw ()
        {
          if (_M_get_root())
            {
    ...
    protected:
      ...
      void _M_check_children( _Link_const_type child, _Link_const_type parent, size_t const level, bool to_the_left )
      {
         ...
         if (child)
         {
	   _Node_compareT compare(level % __K, _M_acc, _M_cmp);
      ...
      {
        if (_Node_compareT(__L % __K, _M_acc, _M_cmp)(__V, __N))
          {
            if (!_S_left(__N))
      ...
      _M_get_j_min( std::pair<_Link_type,size_t> const node, size_t const level) throw ()
      {
        ...
        _Node_compareT compare(node.second % __K, _M_acc, _M_cmp);
      ...
      {
        _Node_compareT compare(__L % __K, _M_acc, _M_cmp);
      ...
      size_t
        _M_count_within_range(_Link_const_type __N, _RegionT const& __REGION,
                             _RegionT const& __BOUNDS,
                             size_t const __L) const throw ()
        {
           size_t count = 0;
      ...
      template <class Visitor>
        ...
        _M_visit_within_range(Visitor visitor,
                             _Link_const_type N, _RegionT const& REGION,
                             _RegionT const& BOUNDS,
      ...
      template <typename _OutputIterator>
        ...
        _M_find_within_range(_OutputIterator __out,
                             _Link_const_type __N, _RegionT const& __REGION,
                             _RegionT const& __BOUNDS,
                             size_t const __L) const throw ()
        {
          if (__REGION.encloses(_S_value(__N)))
       ...
       // And so it goes ...
         ...
         std::pair<const_iterator,distance_type>
        _M_find_nearest( _Link_const_type __N, typename _RegionT::_CenterPt __CENTER,
                             _RegionT const& __BOUNDS,
                             size_t const __L,
